...
import-upstream:
	...
	UPSTREAM_DIR=$(abspath $(UPSTREAM_DIR)) $(MAKE) -f $(abspath $(UPSTREAM_DIR)/Makefile) import-skill VENDOR_DIR=$(abspath .vendor/agent-bridge-clipboard)
	...
	# Fix script paths and re-brand commands
	...
	sed -i 's/agent-bridge-clipboard/gemini-clipboard-bridge/g' commands/cb/*.toml
